      ...
      <div class="calsq day<?= $day == $nowDay ? " today" : "" ?>" data-day="<?= $day ?>">
        ...
        <?php if (isset($events["d"][$day])) {
          foreach ($events["d"][$day] as $eid) { ?>
            <div class="calevent" data-eid="<?= $eid ?>" style="background:<?= $events["e"][$eid]["event_color"] ?>">
              <?= $events["e"][$eid]["event_text"] ?>
            </div>
        <?php if ($day == $events["e"][$eid]["first"]) {
              echo "<div id='event$eid' class='calninja'>" . json_encode($events["e"][$eid]) . "</div>";
            }
          }
        } ?>
      </div>
