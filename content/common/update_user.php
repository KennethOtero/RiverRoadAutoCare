...
<?php
    ...
    if (isset($_SESSION['intUserID'])) {
        //echo "<script>alert('idExists');</script>";
        //Get logged in user
        $userid = (int)$_SESSION['intUserID'];
        ...
        if ($conn) {
            ...
            $sql = "SELECT * FROM TUsers WHERE intUserID =" . $userid;
            $result = $conn->query($sql);
            if ($result) {
                $row = $result->fetch_assoc();
                $_SESSION['strFirstName'] = $row['strFirstName'];
                $_SESSION['strLastName'] = $row['strLastName'];
                $_SESSION['strAddress'] = $row['strAddress'];
                $_SESSION['strApartmentNumber'] = $row['strApartmentNumber'];
                $_SESSION['strCity'] = $row['strCity'];
                $_SESSION['intStateID'] = (int)$row['intStateID'];
                $_SESSION['strZip'] = $row['strZip'];
                $_SESSION['strPhoneNumber'] = $row['strPhoneNumber'];
                $_SESSION['strEmail'] = $row['strEmail'];
                $_SESSION['strPassword'] = $row['strPassword'];
                $_SESSION['strSecurity'] = $row['strSecurity'];
                $_SESSION['intUserTypeID'] = $row['intUserTypeID'];
                $result->free();
                $conn->close();

                //set to page variables
                ...
                $strSecurity = $_SESSION['strSecurity'];
                $intUserTypeID = $_SESSION['intUperTypeID'];
            }
        }
    } else{
        //set to blank
...
<html>
    <head>
        <title>Update User : River Road Auto Care</title>
        <meta charset="UTF-8" />
    ...
    <body>
        ...
        <div class="UpdateUser">
            <h1>Update User</h1>
            <form name="frmLogin" method="post" action="../restricted/process_update_user.php" onsubmit="return validateForm()">
                ...
                <select id="selState" name="selState" required>
                    <?php
                        // Connect to database
                        $servername = "itd2.cincinnatistate.edu";
                        $username = "kjotero2";
                ...
                <input type="text" name="txtPhoneNumber" id="txtPhoneNumber" maxlength="100" value="<?php echo $strPhoneNumber;?>" placeholder="Enter your phone number" required>
                ...
                <span class="details">Security Question Answer: What City Were You Born In?</span>
                <input type="text" name="txtSecurity" id="txtSecurity" placeholder="Enter security answer" value="<?php echo $strSecurity;?>" required>
                <div class="buttons">
        ...
        <script>
            ...
            function validateForm() {
                ...
                var Phone = document.forms['frmLogin']['txtPhoneNumber'].value.trim();
                ...
                if (Security == "") {
                    alert("Enter the security question answer.");
